...
# generate the HelloService.pb file that we can pass to envoy so that knows the grpc service
# we want to expose
protoc -I. -Itarget/protoc-dependencies/eda371bc1f97da4afb21d75a94f4d5df --include_imports \
                --include_source_info \
                --descriptor_set_out=HelloService.pb \
...
if [ "$(groups | grep -c docker)" -gt "0" ]; then
    ...
    docker run -d --name grpc-server-envoy -P 51051:51051 \
             -v "$(pwd)/HelloService.pb:/data/HelloService.pb:ro" \
             -v "$(pwd)/envoy-config.yml:/etc/envoy/envoy.yaml:ro" \
             envoyproxy/envoy:f9646c9803d23baddd745fdf1d30ed6519d1d2dd
else
    echo "you are not in the docker group, running with sudo"
    echo "Envoy will run at port 51051 (see envoy-config.yml)"
    sudo docker run -d --name grpc-server-envoy -p 51051:51051 \
             -v "$(pwd)/HelloService.pb:/data/HelloService.pb:ro" \
             -v "$(pwd)/envoy-config.yml:/etc/envoy/envoy.yaml:ro" \
             envoyproxy/envoy:f9646c9803d23baddd745fdf1d30ed6519d1d2dd
fi
